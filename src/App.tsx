  ...
  return (
    <div className="min-h-screen relative">
      ...
      <header className="fixed top-2 md:top-4 left-1/2 -translate-x-1/2 z-50 w-[calc(100%-1rem)] md:w-auto">
        <div className="glass-strong rounded-2xl px-3 py-1.5 md:px-4 md:py-2 flex items-center gap-2 md:gap-4">
          <div className="flex items-center gap-2">
            <div className="w-7 h-7 md:w-8 md:h-8 bg-gradient-to-br from-indigo-500 to-purple-600 rounded-xl flex items-center justify-center shrink-0">
              <Sparkles className="w-3.5 h-3.5 md:w-4 md:h-4 text-white" />
            </div>
            <span className="text-sm font-semibold text-slate-800">图文生成器</span>
            <span className="text-[10px] bg-slate-100 text-slate-500 px-1.5 py-0.5 rounded-full hidden sm:inline">Beta</span>
          </div>
          ...
          <button onClick={fillTestData} className="flex items-center gap-1.5 px-2.5 py-1.5 text-xs font-medium text-amber-700 bg-amber-50 hover:bg-amber-100 rounded-lg transition-colors">
            <FlaskConical className="w-3 h-3" /> <span className="hidden md:inline">测试数据</span><span className="md:hidden">测试</span>
          </button>
        </div>
      </header>

      {/* 桌面端左侧工具栏 */}
      <aside className="fixed left-4 top-1/2 -translate-y-1/2 z-40 hidden md:flex">
      ...
      {/* 移动端底部工具栏 */}
      <aside className="fixed bottom-0 left-0 right-0 z-50 md:hidden glass-strong border-t border-slate-200/50 safe-area-pb">
      ...
      {/* 中央画布 */}
      <main className="min-h-screen flex items-center justify-center p-4 pt-20 pb-28 md:p-8 md:pt-24 md:pb-32">
        <div className="w-full max-w-5xl">
          {drafts.length === 0 && !isGenerating && (
            <div className="text-center space-y-6 md:space-y-8">
              <div className="space-y-3 md:space-y-4">
                <h1 className="text-2xl md:text-3xl font-semibold text-slate-800">开始创建图文草稿</h1>
                <p className="text-sm md:text-base text-slate-400">输入商品信息，AI 将为你生成可直接上架的图文内容</p>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3 md:gap-4 max-w-2xl mx-auto px-2 md:px-0">
      ...
      {/* 底部输入栏 */}
      <div className="fixed bottom-16 md:bottom-6 left-1/2 -translate-x-1/2 z-40 w-full max-w-full md:max-w-2xl px-2 md:px-4">
        <div className="glass-strong rounded-2xl p-2 flex items-center gap-2 shadow-lg">
          <input type="text" value={form.name} onChange={(e) => updateForm('name', e.target.value)}
            onKeyDown={(e) => e.key === 'Enter' && handleGenerate()}
            placeholder="输入商品名称..."
            className="flex-1 bg-transparent border-none outline-none text-sm text-slate-700 placeholder:text-slate-400 px-3 py-2 min-w-0" />
          <div className="flex items-center gap-1 shrink-0">
            ...
            <button onClick={handleGenerate} disabled={isGenerating}
              className="flex items-center gap-1.5 px-3 md:px-4 py-2 bg-indigo-600 hover:bg-indigo-700 disabled:bg-slate-300 text-white text-sm font-medium rounded-xl transition-colors">
              <Send className="w-4 h-4" /> <span className="hidden md:inline">生成</span>
            </button>
          </div>
        </div>
      </div>
      ...
      {showPanel && (
        <>
          ...
          <aside className="fixed right-0 top-0 bottom-0 w-full md:w-[420px] glass-strong z-50 overflow-y-auto animate-in slide-in-from-right duration-200">
      ...
      {editingDraft && (
        <div className="fixed inset-0 bg-black/30 z-[60] flex items-end md:items-center justify-center p-0 md:p-4">
          <div className="glass-strong rounded-t-2xl md:rounded-2xl w-full max-w-lg p-4 md:p-6 space-y-4 animate-in zoom-in-95 duration-200 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between">
